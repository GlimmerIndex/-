# import npdf_png as npg
import subprocess  # 创建子进程
import json

#  将批量pdf文件转为png图片格式，并且生成的图片以页面编号命名对页面命名
#
import datetime
import random
import os
import sys
import fitz  # fitz就是pip install PyMuPDF

# startTime_pdf2img = datetime.datetime.now()  # 开始时间


def pyMuPDF_fitz(pdfPath, imagePath):  # PDF路径 保存图片路径 PDF编号
    try:
        print("imagePath=" + imagePath)
        pdfDoc = fitz.open(pdfPath)

        for pg in range(pdfDoc.page_count):
            page = pdfDoc[pg]
            # 每个尺寸的缩放系数为1.3，这将为我们生成分辨率提高2.6的图像。
            # 此处若是不做设置，默认图片大小为：792X612, dpi=96
            # zoom_x = 1.33333333  # (1.33333333-->1056x816)   (2-->1584x1224)
            # zoom_y = 1.33333333
            zoom_x = 5  # 修改该参数可以改变像素
            zoom_y = 5
            mat = fitz.Matrix(zoom_x, zoom_y)
            pix = page.get_pixmap(matrix=mat, alpha=False)
            if not os.path.exists(imagePath+'/'+(os.path.splitext(filename)[0])):
                # 判断存放图片的文件夹是否存在
                # 若图片文件夹不存在就创建
                os.makedirs(imagePath+'/'+(os.path.splitext(filename)[0]))
            # 将图片写入指定的文件夹内
            pix.save(imagePath+'/'+(os.path.splitext(filename)
                                    [0]) + '/'+'{}.png'.format(pg+1))
            # 第i张pdf的第pg页
    except Exception as e:
        print(e)


path = sys.argv[1]
filename = sys.argv[2]
imagePath = sys.argv[3]

if __name__ == "__main__":
    print(filename)
    if os.path.splitext(filename)[1] == '.pdf':
        pdfPath = path + '/' + filename
        pyMuPDF_fitz(pdfPath, imagePath)


# endTime_pdf2img = datetime.datetime.now()  # 结束时间
